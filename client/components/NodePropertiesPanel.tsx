...
export default function NodePropertiesPanel() {
  ...
  useEffect(() => {
    if (selectedNodeData) {
      const defaultConfig = defaultNodeConfigs[selectedNodeData.type as keyof typeof defaultNodeConfigs] || [];
      const existingConfig = selectedNodeData.data.config || {};
      
      const config = defaultConfig.map(prop => ({
        ...prop,
        value: existingConfig[prop.id] !== undefined ? existingConfig[prop.id] : prop.value
      }));
  ...
  const updateProperty = (propertyId: string, value: any) => {
    setNodeConfig(prev => prev.map(prop => 
  ...
  const updateArrayItem = (propertyId: string, index: number, value: string) => {
    setNodeConfig(prev => prev.map(prop => 
      prop.id === propertyId ? { 
        ...prop, 
        value: prop.value.map((item: string, i: number) => i === index ? value : item)
      } : prop
    ));
    setHasChanges(true);
  };
  ...
  const renderPropertyInput = (property: NodePropertyConfig) => {
    switch (property.type) {
      ...
      case 'number':
        return (
          <Input
            type="number"
            value={property.value}
            onChange={(e) => updateProperty(property.id, parseFloat(e.target.value) || 0)}
            className="bg-matrix-purple-800/30 border-matrix-purple-600/50"
          />
        );
      ...
      case 'select':
        return (
          <Select value={property.value} onValueChange={(value) => updateProperty(property.id, value)}>
          ...
          <div className="space-y-2">
            {property.value.map((item: string, index: number) => (
              <div key={index} className="flex items-center gap-2">
                <Input
                  value={item}
                  onChange={(e) => updateArrayItem(property.id, index, e.target.value)}
                  className="bg-matrix-purple-800/30 border-matrix-purple-600/50"
                />
                <Button
                  size="sm"
    ...
    return (
      <div className="h-full flex items-center justify-center text-center p-8">
        <div>
          ...
          <h3 className="text-lg font-medium text-matrix-gold-300 mb-2">No Node Selected</h3>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="h-14 glass-panel border-b border-matrix-purple-600/30 flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-matrix-gold-300">{selectedNodeData.data.label}</h3>
            <p className="text-xs text-matrix-purple-400 capitalize">{selectedNodeData.type} Node</p>
          </div>
        </div>
        
        <div className="flex items-center gap-1">
          <Button size="sm" variant="ghost" onClick={duplicateNode} className="h-8 w-8 p-0">
            <Copy className="h-3 w-3" />
          </Button>
          <Button size="sm" variant="ghost" onClick={deleteNode} className="h-8 w-8 p-0 text-red-400 hover:bg-red-500/20">
          ...
          <ScrollArea className="flex-1 px-4 py-4">
            <TabsContent value="properties" className="m-0 space-y-4">
              ...
              <div className="glass-panel rounded-lg p-4">
                <h4 className="text-sm font-medium text-matrix-gold-300 mb-3">Basic Properties</h4>
                <div className="space-y-3">
                  <div>
                    <Label className="text-xs text-matrix-purple-300">Node Label</Label>
                    <Input
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Label className="text-xs text-matrix-purple-300">X Position</Label>
                      <Input
                        type="number"
                        ...
                        onChange={(e) => dispatch({
                          ...
                          payload: {
                            id: selectedNodeData.id,
                            position: { ...selectedNodeData.position, x: parseInt(e.target.value) || 0 }
                          }
                        })}
                        className="bg-matrix-purple-800/30 border-matrix-purple-600/50"
                      />
                    ...
                    <div>
                      <Label className="text-xs text-matrix-purple-300">Y Position</Label>
                      <Input
                        type="number"
                        ...
                        onChange={(e) => dispatch({
                          ...
                          payload: {
                            id: selectedNodeData.id,
                            position: { ...selectedNodeData.position, y: parseInt(e.target.value) || 0 }
                          }
                        })}
                        className="bg-matrix-purple-800/30 border-matrix-purple-600/50"
                      />
              ...
              {/* Advanced Properties */}
              <div className="glass-panel rounded-lg p-4">
                <h4 className="text-sm font-medium text-matrix-gold-300 mb-3">Advanced Configuration</h4>
                <div className="space-y-4">
                  {nodeConfig.map((property) => (
                    <div key={property.id}>
                      <div className="flex items-center justify-between mb-2">
                        <Label className="text-xs text-matrix-purple-300">{property.name}</Label>
                        {property.required && (
                          <Badge variant="outline" className="text-xs h-5 border-red-400/50 text-red-300">
                            Required
                          </Badge>
                        )}
                      </div>
                      ...
                      {property.description && (
                        <p className="text-xs text-matrix-purple-400 mt-1">{property.description}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <TabsContent value="connections" className="m-0">
              <div className="glass-panel rounded-lg p-4">
                <h4 className="text-sm font-medium text-matrix-gold-300 mb-3">Node Connections</h4>
                
                {/* Input Connections */}
                {selectedNodeData.data.inputs && (
                  <div className="mb-4">
                    <h5 className="text-xs font-medium text-matrix-purple-300 mb-2">Inputs</h5>
                    <div className="space-y-2">
                      {selectedNodeData.data.inputs.map((input, index) => (
                        <div key={input} className="flex items-center gap-2 p-2 glass-dark rounded">
                          <div className="w-3 h-3 rounded-full bg-matrix-purple-400 border-2 border-matrix-purple-300"></div>
                          <span className="text-sm text-matrix-purple-200">{input}</span>
                          <Badge variant="outline" className="ml-auto text-xs border-matrix-purple-600/50">
                            Input {index + 1}
                          </Badge>
                        </div>
                      ))}
                ...
                {selectedNodeData.data.outputs && (
                  <div>
                    <h5 className="text-xs font-medium text-matrix-purple-300 mb-2">Outputs</h5>
                    <div className="space-y-2">
                      {selectedNodeData.data.outputs.map((output, index) => (
                        <div key={output} className="flex items-center gap-2 p-2 glass-dark rounded">
                          <div className="w-3 h-3 rounded-full bg-matrix-gold-400 border-2 border-matrix-gold-300"></div>
                          <span className="text-sm text-matrix-purple-200">{output}</span>
                          <Badge variant="outline" className="ml-auto text-xs border-matrix-gold-600/50 text-matrix-gold-300">
                            Output {index + 1}
                          </Badge>
                        </div>
                      ))}
            ...
            <TabsContent value="code" className="m-0">
              <div className="glass-panel rounded-lg p-4">
                <h4 className="text-sm font-medium text-matrix-gold-300 mb-3">Generated Code Preview</h4>
                <div className="bg-matrix-dark/50 rounded p-3 font-mono text-sm">
                  <pre className="text-matrix-purple-200 whitespace-pre-wrap">
                    {selectedNodeData.data.code || '# Code will be generated based on node configuration'}
                  </pre>
                </div>
              </div>
            </TabsContent>
          ...
          {hasChanges && (
            <div className="h-14 glass-panel border-t border-matrix-purple-600/30 flex items-center justify-between px-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" onClick={saveChanges} className="bg-gradient-to-r from-matrix-gold-500 to-matrix-gold-600 hover:from-matrix-gold-600 hover:to-matrix-gold-700 text-matrix-dark">
