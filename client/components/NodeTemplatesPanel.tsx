...
const nodeTemplates: NodeTemplate[] = [
  ...
  // Processing Templates
  {
    ...
    config: { removeSpaces: true, toLowerCase: true, removeSpecialChars: false },
...
const categories = ['All', 'Input', 'Processing', 'AI', 'API', 'Logic', 'Output'];
...
export default function NodeTemplatesPanel() {
  ...
  const addNodeFromTemplate = (template: NodeTemplate, position?: { x: number; y: number }) => {
    const newNode = {
  ...
  const renderTemplateCard = (template: NodeTemplate) => {
      ...
      return (
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded bg-gradient-to-r ${template.color} flex items-center justify-center flex-shrink-0`}>
              <Icon className="h-5 w-5 text-white" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-sm font-medium text-matrix-gold-300 truncate">{template.name}</h3>
                <Badge variant="outline" className="text-xs border-matrix-purple-600/50">
                  {template.category}
                </Badge>
                {template.isCustom && (
              ...
              <p className="text-xs text-matrix-purple-400 truncate">{template.description}</p>
              <div className="flex items-center gap-2 mt-1">
                <div className="flex items-center gap-1">
                  <Star className="h-3 w-3 text-matrix-gold-400" />
                  <span className="text-xs text-matrix-purple-300">{template.popularity}</span>
                </div>
                <div className="flex gap-1">
                  {template.tags.slice(0, 2).map(tag => (
                    <Badge key={tag} variant="outline" className="text-xs h-4 border-matrix-purple-600/30">
                      {tag}
                    </Badge>
                  ))}
                </div>
    ...
    return (
      ...
      >
        ...
        <div className="mb-3">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-sm font-medium text-matrix-gold-300">{template.name}</h3>
            {template.isCustom && (
              <Badge variant="outline" className="text-xs border-matrix-gold-400/50 text-matrix-gold-300">
                Custom
              </Badge>
          ...
          <p className="text-xs text-matrix-purple-400 line-clamp-2">{template.description}</p>
        </div>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="h-14 glass-panel border-b border-matrix-purple-600/30 flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-matrix-gold-300">Node Templates</h3>
            <p className="text-xs text-matrix-purple-400">{filteredTemplates.length} templates available</p>
          </div>
        </div>
        
        <div className="flex items-center gap-1">
          <Button size="sm" variant="ghost" onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} className="h-8 w-8 p-0">
            {viewMode === 'grid' ? <List className="h-3 w-3" /> : <Grid3X3 className="h-3 w-3" />}
          </Button>
          <Button size="sm" variant="ghost" onClick={exportTemplates} className="h-8 w-8 p-0">
            <Download className="h-3 w-3" />
          </Button>
          <Button size="sm" variant="ghost" onClick={() => fileInputRef.current?.click()} className="h-8 w-8 p-0">
      ...
      <ScrollArea className="flex-1 p-4">
        ...
        {filteredTemplates.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-matrix-gold-300 mb-2">No Templates Found</h3>
