    ...
    switch (node.type) {
      case "input":
        if (node.data.label.toLowerCase().includes("file")) {
          ...
          lines.push(
            `        ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_value = f.read()`,
          );
        } else {
          lines.push("    # User input node");
          lines.push(
            `    ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_value = input("${node.data.label}: ")`,
          );
        }
        break;
      ...
      case "function":
        const inputVars = connections
          ...
          .map(
            (conn) =>
              varMap.get(`${conn.source}_${conn.sourceOutput}`) || "data",
          )
          .join(", ");
        ...
        if (node.data.label.toLowerCase().includes("process")) {
          lines.push(
            `    ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = process_data(${inputVars || "None"})`,
          );
        } else if (node.data.label.toLowerCase().includes("validate")) {
          lines.push(
            `    ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = validate_input(${inputVars || "None"})`,
          );
        } else if (node.data.label.toLowerCase().includes("ai")) {
          lines.push(
            `    ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = ai_enhance(${inputVars || "None"})`,
          );
        } else {
          lines.push(
            `    ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = ${inputVars || "None"}`,
          );
        }
        break;
      ...
      case "api":
        ...
        lines.push(
          `    ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = response.json()`,
        );
        break;
      ...
      case "logic":
        ...
        lines.push(
          `        ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = "condition_true"`,
        );
        lines.push("    else:");
        lines.push(
          `        ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = "condition_false"`,
        );
        break;
      ...
      case "output":
        const outputVars = connections
          ...
          .map(
            (conn) =>
              varMap.get(`${conn.source}_${conn.sourceOutput}`) || "result",
          )
          .join(", ");
        ...
        if (node.data.label.toLowerCase().includes("file")) {
          ...
          lines.push(
            `    print(f"${node.data.label}: {${outputVars || "result"}}")`,
          );
        }
        break;
    }
  ...
  sortedNodes.forEach((node) => {
    switch (node.type) {
      case "input":
        lines.push(`    // ${node.data.label} node`);
        lines.push(
          `    const ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_value = await this.getUserInput("${node.data.label}: ");`,
        );
        break;
      ...
      case "function":
        ...
        if (node.data.label.toLowerCase().includes("process")) {
          lines.push(
            `    const ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = await this.processData(user_input_value);`,
          );
        } else {
          lines.push(
            `    const ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = user_input_value;`,
          );
        }
        break;
      ...
      case "output":
        lines.push(`    // ${node.data.label}`);
        lines.push(
          `    console.log(\`${node.data.label}: \${process_data_result}\`);`,
        );
        break;
    }
  });
  ...
  sortedNodes.forEach((node) => {
    switch (node.type) {
      case "input":
        ...
        lines.push(
          `    std::string ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_value;`,
        );
        lines.push(
          `    std::getline(std::cin, ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_value);`,
        );
        break;
      ...
      case "function":
        ...
        if (node.data.label.toLowerCase().includes("process")) {
          lines.push(
            `    std::string ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = processData(user_input_value);`,
          );
        } else {
          lines.push(
            `    std::string ${node.data.label.toLowerCase().replace(/\s+/g, "_")}_result = user_input_value;`,
          );
        }
        break;
      ...
      case "output":
        lines.push(`    // ${node.data.label}`);
        lines.push(
          `    std::cout << "${node.data.label}: " << process_data_result << std::endl;`,
        );
        break;
    }
  });
